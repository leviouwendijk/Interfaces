...
extension InterfacesFlowSuite {
    static var shellFlow: TestFlow {
        TestFlow(
            ...
            Step("signal termination remains typed") {
                let result = try await Shell(
                    .path(
                        "/bin/sh"
                    )
                ...
                case .signaled(
                    ...
                    try Expect.equal(
                        signal,
                        15,
                        "signal termination remains typed"
                    )
                ...
                case .exited(
                    ...
                    throw TestFlowAssertionFailure(
                        label: "shell.signal.status",
                        message: "expected signaled status",
                        actual: String(
                            code
                        ),
                        expected: "signal 15"
            ...
            Step("timeout preserves Shell timeout error") {
                ...
                try Expect.equal(
                    observedTimeout ?? -1,
                    0.1,
                    "timeout duration remains available"
                )

                try Expect.true(
                    (observedPID ?? 0) > 0,
                    "timeout retains child process identifier"
                )
            }
